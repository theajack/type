var words=["与丐丑专中丰丹为之乌书予云互亓五井亢什仁仂仃仄仅仆仇仉今介仍从仑仓允元公六兮内临举亭亮亲侮侯侵万丈三上下丌个丫丸久乇么义乞也习乡亍于亏亡亿兀凡刃勺千卫叉口囗土士夕大女子便两严串丽乱亨亩伯估伲伴伶伸伺似伽但位低住佐佑体何余佛作促俄俊俏俐俑俗俘俚保俞罗者耶肃股肢肤肥肩信俩俪俭修兹养冒冠剃削前",
  ...
  ["长生果","捉迷藏","红领巾","舍不得","加拿大","少不了","过不去","黑沉沉","量角器","井冈山","亮闪闪","卫生间","穿山甲","亮晶晶","怎么办","夜猫子","共和国","毛毛雨","植树节","一会儿","天安门","自来水","大别山","一大半","包心菜","录像机","燕尾服","男子汉","长白山","数不清","停机坪","三角形","降落伞","发动机","静悄悄","笔记本","黄鼠狼","百家姓","山水画","香喷喷","悄悄话","自行车","交朋友","黄花菜","金灿灿","干豆腐","胖乎乎","黑压压","太平洋","王府井","柏油路","泼水节","够意思","钻空子","笑哈哈","小册子","水汪汪","北京市","胡萝卜","讲故事","一阵风","立交桥","杜鹃花","托儿所","来不及","办公室","国庆节","石灰岩","蜘蛛网","沉甸甸","候车室","打雪仗","气冲冲","台湾省","宇航员","泼冷水","富春江","一刹那","恨不得","普通话","老百姓","后脑勺","孤零零","忍不住","白茫茫","凤尾竹","差不多","一转眼","记忆力","难为情","微生物","绿油油","黑乎乎","工艺品","主席台","显微镜","慢吞吞","放大镜","一眨眼","懒洋洋","急匆匆","俱乐部","齐刷刷","兵马俑","博物馆","笑盈盈","葡萄灰","九寨沟","土地庙","飘飘然","催眠曲","卷铺盖","白皑皑","顶梁柱","葡萄干","糖尿病","暖烘烘","原子核","一锅端","搁得住","敲门砖","敬亭山","兴冲冲","睁眼瞎","昆明湖","颐和园","绿葱葱","万寿山","五粮液","哲学家","煤油灯","手榴弹","不由得","红扑扑","风向标","扭秧歌","火辣辣","摄氏度","绿茵茵","障碍物","水墨画","共产党","凉飕飕","和氏璧","宿营地","弓弩手","威尼斯","军令状","乱哄哄","下马威","炊事员","明晃晃","黑糊糊","麦芽糖","霎时间","里程碑","欣欣然","本命年","风景线","口头禅","麻麻亮","冷飕飕","空荡荡","试帖诗","迫击炮","鹅卵石","戈壁滩","汨罗江","霓虹灯","白洋淀","颤巍巍","文绉绉","冼星海","金銮殿","挖墙脚","可怜相","佼佼者","潜意识","打牙祭","出风头","闭门羹","笑吟吟","度假村","侦察兵","年轻人","想象力","轩辕氏","国子监","万户侯","挪窝儿","球面镜","不送气","自治县","可怜见","吨海里","动物学","贱骨头","小金库","豆嘴儿","枪榴弹","脑门儿","更年期","现世报","唯心论","毛细管","检字法","大约摸","马蜂窝","讨人嫌","进行曲","开尔文","中立国","门鼻儿","协奏曲","打野外","再生产","溜溜儿","古生物","同盟军","见世面","辩护人","冥王星","橡皮膏","小性儿","清道夫","动肝火","八月节","画等号","小老婆","珂罗版","秤盘子","不要脸","高岭土","公平秤","大杂烩","崩龙族","火车头","过家伙","弹簧门"],
...
function Enemy(){
  this.w=30;
  this.h=30;
  this.x=J.getRandom(0,cw);
  this.y=-this.h;
  this.type=J.getRandom(0,words.length-1);
  this.words=words[this.type][J.getRandom(0,words[this.type].length-1)];
  this.pinyin=this.words.spell("low");
  this.hp=this.pinyin.length;
  this.index=0;
};Enemy.prototype.act=function(){
  var rate=this.speed/countDis(this,player);
  this.x+=(rate*(player.x-this.x));
  this.y+=(rate*(player.y-this.y));
  if(this.touchTarget()){
    J.show("over");
  }
  this.draw();
};Enemy.prototype.draw=function(){
  ctx.font="20px bold MicrosofeYahei";
  ctx.fillStyle="#f00";
  ...
  enemys.remove(this);
};Enemy.prototype.check=function(letter){
  if(this.index<this.pinyin.length){
    if(letter==this.pinyin[this.index]){
      this.index++;
      return true;
    }
  }
